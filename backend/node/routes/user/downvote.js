...
const downvote = Router()

downvote.post("/", jwtAuthentication, async (req, res) => {
  ...
  if (type === "question") {
    ...
    if (user.downvotes.includes(id)) {
      ...
      try {
        ...
        return res
          .status(200)
          .json({ message: "downvote removed successfully", id })
      } catch (err) {
    ...
    asker.notifications.push({
      type: "downvote",
      message: `Your question "${question.title}" has received an downvote.`,
    ...
    try {
      ...
      return res
        .status(200)
        .json({ message: "downvote for question successful", id })
    } catch (err) {
      console.error("Error saving question:", err)
      return res.status(500).json({ error: "Internal Server Error" })
    }
    ...
    replier.notifications.push({
      type: "downvote",
      message: `Your answer has received an downvote.`,
      answerId: answer._id,
    ...
    try {
      ...
      return res
        .status(200)
        .json({ message: "downvote for answer successful", id })
    } catch (err) {
